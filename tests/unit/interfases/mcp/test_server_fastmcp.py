...
def test_database_options_rejects_client_engine_configuration() -> None:
    ...
    with pytest.raises(ValueError, match="Custom database configuration is not permitted"):
        options.build()
...
def test_database_options_rejects_unknown_preset() -> None:
    ...
    with pytest.raises(ValueError, match="preset 'production' is not permitted"):
        options.build({})
